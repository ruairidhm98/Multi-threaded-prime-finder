...
static void heapify_down(MinHeap *heap) {
  ...
  while (has_left_child(heap, index)) {
    ...
    if (has_right_child(heap, index) &&
        get_right_child(heap, index) < get_left_child(heap, index))
    ...
    if (heap->elements[index] <= heap->elements[smaller_child_index])
      ...
      swap(heap->elements, index, smaller_child_index);
...
static int get_left_child(MinHeap *heap, int index) {
  return heap->elements[get_left_child_index(index)];
}

/* Helper function, gets the right child of a parent node */
static int get_right_child(MinHeap *heap, int index) {
  return heap->elements[get_right_child_index(index)];
}

/* Helper function, gets the parent of a given node */
static int get_parent(MinHeap *heap, int index) {
  return heap->elements[get_parent_index(index)];
}
