/* Multi-threaded program which finds a certain number of prime numbers
 * concurrently */
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < nthread; i++)
    ...
    if (pthread_create(&generatorThreads[i], NULL, search_region,
                       (void *)&args[i])) {
  ...
  fprintf(stderr, "%lu primes computed in %.4f seconds, %.4f ms/prime\n", limit,
          ...
          1000 * ((t2 - t1) / CLOCKS_PER_SEC) / limit / nthread);
  /* Free heap memory */
  free((void *)args);
  free((void *)args1);
  bb_destroy(bb);
